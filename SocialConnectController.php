...
//        $this->socialite->driver($provider)->deauthorize(\Auth::user()->access_token);
        ...
        if (!isset($email) || $email == '') {
            ...
            Auth::logout();
            return redirect('/login')->with('login-error', "Please login in Facebook with your email id as 'login with Mobile number' is not allowed. And Please logout your account from facebook.");
        }
...
//                dd($user->email);
        ...
        $gender = ($user->user['gender'] == "male") ? "1" : ($user->user['gender'] == "female") ? "2" : "3";
        $id = $user->id;
        $redirect_url = $this->handleSocialConnect($email, $gender, $first_name, $last_name, $id, "google");
        return redirect($redirect_url);
    }
