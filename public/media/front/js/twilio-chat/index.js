...
//            activeChannel.getMessages(1).then(messages => {
            activeChannel.getMessages(1).then(function(messages) {
...
//            activeChannelPage.prevPage().then(page => {
            activeChannelPage.prevPage().then(function(page) {
    ...
    $('#join-channel').on('click', function() {
        $.ajax({
            url: javascript_site_path+'is-chat-start',
            data: {
                'appointment_id': $("#current_appointment_id").val()
            },
            dataType: "html",
        ...
        identity && activeChannel.add(identity);
    ...
    //create channel
    $('#create-appointment-channel').on('click', function() {
        var attributes = {
        ...
        // var friendlyName = "Private Channel";
        var uniqueName = $('#create-channel-unique-name').val();
        // var uniqueName = "private-channel";
// alert(uniqueName); return false;
...
//        alert("You are redirecting.");
        window.location.href = javascript_site_path+"text-chat/"+$("#current_appointment_id").val();
    });
...
function logIn(identity, displayName) {
    fingerprint.get(function(endpointId) {
        request(javascript_site_path + 'getToken?identity=' + identity + '&endpointId=' + endpointId, function(err, res) {
            if (err) {
...
//    let hasReachability = (user.online !== null) && (typeof user.online !== 'undefined');
        var hasReachability = (user.online !== null) && (typeof user.online !== 'undefined');
        var $span = $('<span/>')
...
function updateChannels() {
    var current_channel = $("#current_appointment").val();
                    ...
                    switch (channel.status) {
                        case 'joined':
                           // addJoinedChannel(channel);
                            if(current_channel == channel.friendlyName){
                                // addJoinedChannel(channel);
...
//    let randomEndpointId = Math.random().toString(36).substring(7);
    ...
    var randomEndpointId = Math.random().toString(36).substring(7);
