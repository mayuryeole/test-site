...
@section('content')
    ...
    <section class="h-ecard-page manage-gift-cards">
        <div class="container">
            @if (session('purchase-status'))
                <div class="alert alert-success">
                    {{ session('purchase-status') }}
            ...
            <div class="card-product-image clearfix">
                <div class="card-pro-img">
                    <img @if(isset($giftCard) && count($giftCard)>0)src="{{ url('storage/app/public/gift_card_image').'/'.$giftCard->image }}" @endif alt="image"/>
                </div>
            ...
            <form id="purchase-gift-card-form" name="purchase_gift_card" action="{{ url('/indipay/request') }}" role="form" method="post" enctype="multipart/form-data">
            <div class="card-details">
                {{ csrf_field() }}
                <input type="hidden" id="min-purchase-amt" name="min_purchase_amt" value="{{ $giftCard->price }}">
                <input type="hidden" id="ip-gift-card-id" name="gift_card_id" value="{{ $giftCard->id }}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <input id="amount" name="amount" type="text" class="form-control" placeholder=" minimum purchase amount {{ $giftCard->price }}"/>
                                    </div>
                                </div>
                            </div>
                        </div>
