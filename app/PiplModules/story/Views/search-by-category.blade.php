	...
	@foreach($posts as $key => $post)
        ...
        @if($post->story_image)
          <div class="col-md-1 text-center">
            	<img src="{{ asset('storageasset/story/thumbnails/'.$post->story_image) }}" class="img-responsive thumbnail" />
        	</div>
        @endif
           
        <div class=" @if($post->story_image) col-md-11 @else col-md-12 @endif">
        <h4><a href="{{ url('/story/'.$post->story_url) }}" title="Click to view the post" target="new">{{$post->translateOrDefault(\App::getLocale())->title}}</a></h4>
        {{$post->translateOrDefault(\App::getLocale())->short_description}}
        <br /><br />
        <div><i class="fa fa-calendar"></i> {{ $post->updated_at->format('M d, Y h:i A')}} &nbsp; <i class="fa fa-tag"></i> @foreach($post->tags as $tag) <a href="{{ url('/story/tags/'.$tag->slug) }}"><i class="badge">{{$tag->name}}</i></a> @endforeach</div>
        </div>
        </div>
