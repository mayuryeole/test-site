                       ...
                       // $post->post_url = str_slug($request->url);
                        ...
                        if ($request->hasFile('attachments')) {
                            ...
                            foreach ($uploaded_files as $uploaded_file) {
                                ...
                                $new_file_name = str_replace(".", "-", microtime(true)) . "." . $extension;
                                Storage::put('public/blog/attachments/' . $new_file_name, file_get_contents($uploaded_file->getRealPath()));
...
//                     'post_url' => str_slug($request->url),
    ...
    public function createBlogCategories(Request $request) {
        if ($request->method() == "GET") {
            $existing_categories = PostCategory::where('parent_id',null)->withTranslation()->get();
...
//                        'slug.unique' => 'This url is already exists',
            ...
            if ($validate_response->fails()) {
                ...
                $created_category = PostCategory::create(array('created_by' => Auth::user()->id, 'parent_id' => $request->parent_id, 'slug' => $request->slug));
    ...
    public function viewPost(Request $request, $post_url) {
        ...
        if ($page && $page->post_status) {
            ...
            if ($request->method() == "GET") {
                ...
                $comments=  PostComment::where('post_id',$page->id)->first();
//                dd($page_information->post->allow_comments);
//                $c=$page_information->post->allow_comments;
                return view('blog::view-post', array('comments'=>$comments,'page' => $page, 'page_information' => $page_information));
            } else {
                // Post Comments
...
//                        dd($uploaded_files);
                        foreach ($uploaded_files as $uploaded_file) {
                            ...
                            $new_file_name = str_replace(".", "-", microtime(true)) . "." . $extension;
                            Storage::put('/public/blog/comment_attachment/' . $new_file_name, file_get_contents($uploaded_file->getRealPath()));
...
//                    dd($attachments);
    ...
    public function viewBlogPosts() {
        $posts = Post::translatedIn(\App::getLocale())->where('post_status', '1')->orderBy('updated_at', 'desc')->paginate(5);
