				...
				<div class="col-md-12">
					...
					<div class="portlet box grey-cascade">
						<div class="portlet-title">
							...
							@if(Auth::user()->hasPermission('create.artist')==true || Auth::user()->isSuperadmin())
                           	
                                                            <div class="row">
									<div class="col-md-6">
										<div class="btn-group">
											<a href="{{url('admin/create-registered-artist')}}" id="sample_editable_1_new" class="btn green">
...
<!--                                                                 <th>youtube_link</th>
                                                      ...
                                                      <input type="button" onclick='javascript:deleteAll("{{url('/admin/delete-selected-artist')}}")' name="delete" id="delete" value="Delete Selected" class="btn btn-danger">
						</div>
					</div>
                </div>
         </div>
...
$(function() {
    $('#tbl_regusers').DataTable({
        ...
        serverSide: true,
        //bStateSave: true,
        ajax: {"url":'{{url("/admin/list-artist-data")}}',"complete":afterRequestComplete},
        columnDefs: [{
        "defaultContent": "-",
        "targets": "_all"
      }],
            ...
            {data:   "Update",
              render: function ( data, type, row ) {
                    ...
                    if ( type === 'display' ) {
                        ...
                        return '<a class="btn btn-sm btn-primary" href="{{url("/admin/artist/update")}}/'+row.id+'">Update</a>';
                    }
                    return data;
                },
                  "orderable": false,
            ...
            {data:   "Delete",
              render: function ( data, type, row ) {
                    ...
                    if ( type === 'display' ) {
                        ...
                        return '<form id="delete_user_'+row.id+'" method="post" action="{{url("/admin/delete-artist")}}/'+row.id+'">{{ method_field("DELETE") }} {!! csrf_field() !!}<button onclick="confirmDelete('+row.id+')" class="btn btn-sm btn-danger" type="button">Delete</button></form>';
                    }
                    return data;
                },
                  "orderable": false,
